  ...
  for (const [themeName, { extend, layout, colors }] of Object.entries(themes)) {
    ...
    for (const [colorName, colorValue] of Object.entries(flatColors)) {
      ...
      try {
        ...
        const twtColorVariable = `--${prefix}-${colorName}`
        const twtOpacityVariable = `--${prefix}-${colorName}-opacity`

        // set the css variable in '@layer utilities'
        resolved.utilities[cssSelector]![twtColorVariable] = `${h} ${s}% ${l}%`
        // if an alpha value was provided in the color definition, store it in a css variable
        if (typeof defaultAlphaValue === 'number') {
          resolved.utilities[cssSelector]![twtOpacityVariable] = defaultAlphaValue.toFixed(2)
        }
        // set the dynamic color in tailwind config theme.colors
        resolved.colors[colorName] = ({ opacityVariable, opacityValue }) => {
          // if the opacity is set  with a slash (e.g. bg-primary/90), use the provided value
          if (!isNaN(+opacityValue)) {
            return `hsl(var(${twtColorVariable}) / ${opacityValue})`
          }
          // if no opacityValue was provided (=it is not parsable to a number)
          // the twtOpacityVariable (opacity defined in the color definition rgb(0, 0, 0, 0.5)) should have the priority
          // over the tw class based opacity(e.g. 'bg-opacity-90')
          // This is how tailwind behaves as for v3.2.4
          if (opacityVariable) {
            return `hsl(var(${twtColorVariable}) / var(${twtOpacityVariable}, var(${opacityVariable})))`
          }

          return `hsl(var(${twtColorVariable}) / var(${twtOpacityVariable}, 1))`
        }
      } catch (error: any) {
...
export const createTheme = (config: PluginConfig = {}): ReturnType<typeof plugin> => {
  const {
    themes: themeObject = {},
    defaultTheme = 'light',
    layout: userLayout,
